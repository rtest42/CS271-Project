...
def main():
    ...
    # --- Actual ---
    im0 = axes[0].imshow(true, cmap="nipy_spectral", origin="lower", extent=[float(ds.lon.min() - 180), float(ds.lon.max() - 180), float(ds.lat.min()), float(ds.lat.max())])
    axes[0].set_title("Actual Temperature")
    plt.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04)

    # --- Predicted ---
    im1 = axes[1].imshow(pred, cmap="nipy_spectral", origin="lower", extent=[float(ds.lon.min() - 180), float(ds.lon.max() - 180), float(ds.lat.min()), float(ds.lat.max())])
    axes[1].set_title("Predicted Temperature")
    plt.colorbar(im1, ax=axes[1], fraction=0.046, pad=0.04)

    # --- Error (Difference) ---
    im2 = axes[2].imshow(error, cmap="bwr", origin="lower", extent=[float(ds.lon.min() - 180), float(ds.lon.max() - 180), float(ds.lat.min()), float(ds.lat.max())])
