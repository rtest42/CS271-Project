...
class Conv2dLSTMCell(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, kernel_size: int | tuple[int, int], bias: bool = True):
    ...
    def forward(self, input_, hx=None):
        # Inputs:
        #       input_: of shape (batch_size, input_size, height_size, width_size)
        ...
        if self.Wc is None:
            if torch.cuda.is_available():
                self.Wc = nn.Parameter(torch.zeros((1, self.hidden_size * 3, input_.size(2), input_.size(3))).cuda())
            else:
                self.Wc = nn.Parameter(torch.zeros((1, self.hidden_size * 3, input_.size(2), input_.size(3))))

        if hx is None:
            hx = input_.new_zeros(input_.size(0), self.hidden_size, input_.size(2), input_.size(3))
            hx = (hx, hx)
        hx, cx = hx

        gates = self.x2h(input_) + self.h2h(hx)
...
class Conv2dRNNCell(nn.Module):
    ...
    def forward(self, input_, hx=None):
        ...
        hy = (self.x2h(input_) + self.h2h(hx))
